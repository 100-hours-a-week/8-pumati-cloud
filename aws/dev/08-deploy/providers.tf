...
# ArgoCD Provider 설정 - ArgoCD 서버가 이미 배포된 상태에서 사용
provider "argocd" {
  server_addr = "argocd.${local.domain_name}"  # ArgoCD 서버 주소
  username    = "admin"                        # ArgoCD admin 사용자명
  password    = var.argocd_admin_password      # ArgoCD admin 비밀번호 (변수로 받음)
  insecure    = false                          # HTTPS 사용
  grpc_web    = true                           # gRPC-Web 사용 (브라우저 호환성)
} 