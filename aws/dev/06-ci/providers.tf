...
locals {
  ...
  # 01-static에서 생성한 Jenkins EBS 볼륨 정보
  jenkins_ebs_volume_id = data.terraform_remote_state.static.outputs.jenkins_ebs_info.volume_id
  jenkins_ebs_availability_zone = data.terraform_remote_state.static.outputs.jenkins_ebs_info.availability_zone
  jenkins_ebs_size = data.terraform_remote_state.static.outputs.jenkins_ebs_info.size
  jenkins_ebs_type = data.terraform_remote_state.static.outputs.jenkins_ebs_info.type
  jenkins_ebs_encrypted = data.terraform_remote_state.static.outputs.jenkins_ebs_info.encrypted
  jenkins_ebs_iops = data.terraform_remote_state.static.outputs.jenkins_ebs_info.iops
  jenkins_ebs_throughput = data.terraform_remote_state.static.outputs.jenkins_ebs_info.throughput
  
  # 01-static에서 생성한 기타 정보
  acm_certificate_arn = data.terraform_remote_state.static.outputs.acm_certificate_arn
}
