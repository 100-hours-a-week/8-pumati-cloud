...
data "terraform_remote_state" "common" {
  ...
  config = {
    ...
    key    = "aws/dev/00-common/terraform.tfstate"
...
### 변수 사용
변수를 사용할 때는 `locals` 블록을 통해 접근합니다:

```hcl
resource "aws_s3_bucket" "example" {
  bucket = "${locals.project_name}-${locals.environment}-bucket"
  
  tags = merge(
    locals.common_tags,
    {
      Name = "${locals.project_name}-${locals.environment}-bucket"
    }
  )
}
```
...
```bash
# Owner 태그 변경 예시
terraform apply -var='common_tags={"ManagedBy":"Terraform","Project":"ktb8team","Environment":"dev","Owner":"rowan"}'
```
...
- 백엔드 설정에서 올바른 경로(`aws/dev/common/terraform.tfstate`)를 사용하고 있는지 확인하세요.