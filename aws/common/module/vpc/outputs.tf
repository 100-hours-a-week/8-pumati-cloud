...
output "service_subnet_cidr_block" {
  description = "서비스 서브넷 CIDR 블록 (사용되는 경우)"
  value       = try(aws_subnet.service[0].cidr_block, null)
}
...
output "db_subnet_cidr_block" {
  description = "DB 서브넷 CIDR 블록 (사용되는 경우)"
  value       = try(aws_subnet.db[0].cidr_block, null)
}

output "db_route_table_id" {
  description = "DB 서브넷 라우팅 테이블 ID (사용되는 경우)"
