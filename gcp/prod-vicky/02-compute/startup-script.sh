...
gsutil cp gs://ktb8team-static-storage-prod-vicky/cloudflare/${TUNNEL_UUID}.json /etc/cloudflared/llm-tunnel.json
gsutil cp gs://ktb8team-static-storage-prod-vicky/cloudflare/cert.pem /root/.cloudflared/cert.pem
...
ingress:
  - hostname: prod-vicky.mydairy.my
...
# 3. 이미지 pull & 컨테이너 실행
log_message "▶ AI Badge 서비스 이미지 다운로드 중..."
IMAGE="asia-east1-docker.pkg.dev/ktb8team-458916/ktb8team/prod/badge"
...
if docker pull "$IMAGE:latest"; then
  ...
  curl -H "Content-Type: application/json" \
       -X POST \
       -d "{\"content\": \"🚨 AI Badge 이미지 다운로드 실패: 호스트 $(hostname)에서 $IMAGE:latest를 가져올 수 없습니다.\"}" \
       "${WEBHOOK_URL}"
  exit 1
fi

# 기존 컨테이너 중지 및 삭제
log_message "▶ 기존 AI Badge 서비스 컨테이너 정리 중..."
docker stop ai-badge >/dev/null 2>&1 || true
docker rm ai-badge >/dev/null 2>&1 || true

# 새 컨테이너 실행
log_message "▶ AI Badge 서비스 컨테이너 시작 중..."
# Watchtower가 이 컨테이너를 감시할 수 있도록 라벨 추가: -l com.centurylinklabs.watchtower.enable=true
if docker run --gpus all -d --restart unless-stopped --name ai-badge -p 8080:8080 -l com.centurylinklabs.watchtower.enable=true "$IMAGE:latest"; then
  log_message "✅ AI Badge 서비스 컨테이너 실행 성공"
  
  # AI Badge 서비스가 충분히 시작할 시간 제공
  log_message "▶ AI Badge 서비스 초기화 대기 중 (30초)..."
  sleep 30  # 서비스 초기화에 충분한 시간 제공
  
  # 헬스체크 한번만 실행
  log_message "▶ AI Badge 서비스 헬스체크 실행 중..."
  if curl -sf "http://localhost:8080/healthz"; then
    log_message "✅ AI Badge 서비스 응답 확인 (엔드포인트: /healthz)"
  else
    log_message "⚠️ 헬스체크 응답 없음: 서비스가 아직 초기화 중이거나 다른 엔드포인트를 사용할 수 있음"
  ...
  curl -H "Content-Type: application/json" \
       -X POST \
       -d "{\"content\": \"✅ AI Badge 서비스가 시작되었습니다. 호스트: $(hostname), 포트: 8080\"}" \
       "${WEBHOOK_URL}"
else
  log_message "❌ AI Badge 서비스 컨테이너 실행 실패"
  docker logs ai-badge
  
  # 실패 시 로그 수집 (최대 50줄)
  CONTAINER_LOGS="$(docker logs ai-badge 2>&1 | tail -n 50 || echo '로그를 가져올 수 없습니다')"
...
log_message "✅ AI Badge 서비스 설정 완료"
...
# 감시할 컨테이너 이름들 (AI Badge 서비스만)
AI_CONTAINER_NAME="ai-badge"
...
if docker run -d \
  ...
  curl -H "Content-Type: application/json" \
       -X POST \
       -d "{\"content\": \"🚨 Watchtower 컨테이너 시작 실패! (종료 코드: $EXIT_CODE) 호스트: $(hostname). AI Badge 컨테이너 자동 업데이트 작동 불가.\"}" \
