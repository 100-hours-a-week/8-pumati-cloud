...
# 대체 머신 타입용 인스턴스 템플릿 (g2-standard-12)
resource "google_compute_instance_template" "g2_standard_12" {
  name_prefix  = "l4-spot-g2std12-"
  project      = local.project_id
  ...
  tags   = ["l4-spot", "g2-standard-12"]
  labels = merge(local.common_labels, { machine-type = "g2_standard_12" })
...
# Instance Flexibility를 지원하는 Regional MIG (여러 존 자동 시도)
resource "google_compute_region_instance_group_manager" "l4_spot_flexible" {
  name               = "l4-spot-flexible-mig"
  project            = local.project_id
  region             = "asia-east1"  # Zonal 대신 Regional 사용
  ...
  depends_on = [
    ...
    google_compute_instance_template.g2_standard_12,
    google_compute_health_check.l4_spot
  ]

  # Regional MIG는 자동으로 리전 내 모든 존에서 리소스를 찾습니다

  # 여러 버전 설정 - GCP가 자동으로 가용한 것을 선택
  # Regional MIG는 최대 2개 버전만 지원
  ...
  update_policy {
    ...
    max_surge_fixed                = 0  # RECREATE 방식에서는 0이어야 함
    max_unavailable_fixed          = 3  # 존의 개수(3개)와 동일하게 설정
