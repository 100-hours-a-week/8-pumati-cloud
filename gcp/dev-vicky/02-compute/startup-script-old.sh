...
# 최신 커스텀 이미지 정보 가져오기
log_message "▶ 최신 pumati-ai 이미지 확인 중..."
IMAGE_FAMILY="pumati-ai"
IMAGE_PROJECT="$(gcloud config get-value project)"

# 이미지 정보 가져오기
LATEST_IMAGE=$(gcloud compute images describe-from-family $IMAGE_FAMILY --project=$IMAGE_PROJECT --format="value(name)")

if [ -n "$LATEST_IMAGE" ]; then
  log_message "✅ 최신 이미지 확인됨: $LATEST_IMAGE"
  
  # 컨테이너 실행 환경 설정
  log_message "▶ 애플리케이션 실행 준비 중..."
  
  # 이미지에서 필요한 설정 파일 복사 (필요한 경우)
  # cp /path/to/image/config /opt/ai-app/config
  
  # 애플리케이션 실행
  log_message "▶ 애플리케이션 실행 중..."
  docker run -d --gpus all -p 8000:8000 --name pumati-ai \
    -v /opt/ai-app:/app/data \
    --restart unless-stopped \
    gcr.io/$IMAGE_PROJECT/pumati-ai:latest
  
  if [ $? -eq 0 ]; then
    log_message "✅ 애플리케이션 실행 성공"
    
    # 애플리케이션 실행 상태 확인
    sleep 10
    if docker ps | grep -q pumati-ai; then
      log_message "✅ 컨테이너가 정상적으로 실행 중입니다"
      
      # 서비스 상태 알림
      curl -H "Content-Type: application/json" \
         -X POST \
         -d "{\"content\": \"✅ pumati-ai 서비스가 시작되었습니다. 서버: $(hostname), 이미지: $LATEST_IMAGE\"}" \
         "${WEBHOOK_URL}"
    else
      log_message "❌ 컨테이너가 시작되었으나 실행 중이 아닙니다"
      docker logs pumati-ai
      
      # 실패 알림
      curl -H "Content-Type: application/json" \
         -X POST \
         -d "{\"content\": \"🚨 pumati-ai 컨테이너가 비정상 종료되었습니다. 서버: $(hostname)\"}" \
         "${WEBHOOK_URL}"
    fi
  else
    log_message "❌ 애플리케이션 실행 실패"
    
    # 대체 포트로 시도
    log_message "▶ 대체 포트로 재시도 중..."
    docker run -d --gpus all -P --name pumati-ai \
      -v /opt/ai-app:/app/data \
      --restart unless-stopped \
      gcr.io/$IMAGE_PROJECT/pumati-ai:latest
      
    if [ $? -eq 0 ]; then
      CONTAINER_PORT=$(docker port pumati-ai | head -n 1 | awk '{print $3}' | cut -d':' -f2)
      log_message "✅ 애플리케이션이 대체 포트에서 실행됨: $CONTAINER_PORT"
      
      # 성공 알림 (대체 포트)
      curl -H "Content-Type: application/json" \
         -X POST \
         -d "{\"content\": \"✅ pumati-ai 서비스가 대체 포트 $CONTAINER_PORT 에서 시작되었습니다. 서버: $(hostname)\"}" \
         "${WEBHOOK_URL}"
    else
      log_message "❌ 모든 시도 실패"
      
      # 실패 알림
      curl -H "Content-Type: application/json" \
         -X POST \
         -d "{\"content\": \"🚨 pumati-ai 서비스 시작 실패: 서버 $(hostname)에서 컨테이너를 실행하지 못했습니다.\"}" \
         "${WEBHOOK_URL}"
    fi
  fi
else
  log_message "❌ 이미지 정보를 가져올 수 없습니다: $IMAGE_FAMILY "
  
  # 실패 알림
  curl -H "Content-Type: application/json" \
       -X POST \
       -d "{\"content\": \"🚨 pumati-ai 이미지를 찾을 수 없습니다. 서버: $(hostname)\"}" \
       "${WEBHOOK_URL}"
fi
