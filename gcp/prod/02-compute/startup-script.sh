...
docker run -d --restart=unless-stopped \
  ...
  --gpus all \
  -p 9400:9400 \
  -e DCGM_EXPORTER_KUBERNETES=1 \
  nvidia/dcgm-exporter:3.3.0-3.4.0-ubuntu22.04
...
if docker run --gpus all -d --restart unless-stopped --name ai -p 8080:8080 "$IMAGE:latest"; then
  ...
  # 헬스체크 로직 개선 (중괄호 없이 구현)
  log_message "▶ AI 서비스 헬스체크 시작..."
  ...
  # 첫 번째 실패시 /api 엔드포인트 확인
  if [ "$HEALTH_SUCCESS" = false ]; then
    for i in $(seq 1 10); do
      if curl -sf "http://localhost:8080/api"; then
        log_message "✅ AI 서비스 응답 확인 (엔드포인트: /api)"
        HEALTH_SUCCESS=true
        break
      fi
      log_message "⏳ 헬스체크 재시도 $i/10 (엔드포인트: /api)..."
      sleep 5
    done
  fi
  ...
  if [ "$HEALTH_SUCCESS" = false ]; then
    for i in $(seq 1 10); do
      if curl -sf "http://localhost:8080/"; then
        log_message "✅ AI 서비스 응답 확인 (엔드포인트: /)"
        HEALTH_SUCCESS=true
        break
      fi
      log_message "⏳ 헬스체크 재시도 $i/10 (엔드포인트: /)..."
      sleep 5
    done
  fi
...
# 환경 설정 - 환경변수는 systemd 서비스에서 전달받음
...
# 원격 다이제스트 가져오는 함수 개선 - HTTP API 직접 호출 방식
get_remote_digest() {
  local image_name="$1"
  local digest=""
  ...
  # HTTP API 직접 호출로 Schema-2 Manifest 다이제스트 추출
  digest=$(curl -sI \
    -H "Authorization: Bearer $TOKEN" \
    -H "Accept: application/vnd.docker.distribution.manifest.v2+json" \
    "https://$REGISTRY/v2/$PROJECT/$REPO/manifests/latest" | \
...
# 로컬 다이제스트 가져오는 함수 - RepoDigests 항목에서 직접 추출
get_local_digest() {
  local image_name="$1"
  local digest=""
  
  # 이미지가 로컬에 존재하는지 먼저 확인
  if ! docker image inspect "$image_name" &>/dev/null; then
    echo ""
    return
  fi
  
  # RepoDigests에서 첫 번째 항목의 다이제스트 부분만 추출
  digest=$(docker inspect --format='{{index .RepoDigests 0}}' "$image_name" 2>/dev/null | cut -d@ -f2 || echo "")
  
  # 다이제스트가 추출되지 않은 경우 빈 문자열 반환
  if [ -z "$digest" ]; then
    echo ""
    return
  fi
...
# 현재 실행 중인 컨테이너의 이미지 ID 저장 (롤백용)
save_working_image() {
  local container_id="$1"
  if [ -n "$container_id" ]; then
    local current_image_id=$(docker inspect --format='{{.Image}}' "$container_id" 2>/dev/null)
    if [ -n "$current_image_id" ]; then
      echo "$current_image_id" > /tmp/last_working_image_id.txt
      echo "[$TIMESTAMP] ✅ 작동 중인 이미지 ID 저장됨: $current_image_id" >> "$LOGFILE"
    fi
  fi
}
