...
# CI/CD로 설치된 애플리케이션 확인
APP_DIR="/opt/ktb-chat-backend/backend"

if [ -d "$APP_DIR" ]; then
    echo "✅ CI/CD 애플리케이션 디렉토리 발견: $APP_DIR"
    cd $APP_DIR
    
    # 환경변수 파일 업데이트 (기존 설정 유지하면서 추가)
    cat >> .env << EOF
# Terraform에서 추가된 환경변수
PORT=${app_port}
...
EOF
    ...
    # PM2 프로세스 재시작 (CI/CD에서 설정된 이름 사용)
    pm2 restart ktb-chat-backend || pm2 start ecosystem.config.js
    
else
    echo "❌ CI/CD 앱이 없음, 레거시 모드로 실행"
    # 기존 /opt/app 방식으로 폴백
fi
...
check_resources() {
    ...
    [ "$CPU_USAGE" -gt "$CPU_THRESHOLD" ] && send_system_alert "⚠️ 높은 CPU 사용률" "CPU: $${CPU_USAGE}% (임계값: $${CPU_THRESHOLD}%)" 16776960 "backend"
    [ "$MEMORY_USAGE" -gt "$MEMORY_THRESHOLD" ] && send_system_alert "⚠️ 높은 메모리 사용률" "메모리: $${MEMORY_USAGE}% (임계값: $${MEMORY_THRESHOLD}%)" 16776960 "backend"
    [ "$DISK_USAGE" -gt "$DISK_THRESHOLD" ] && send_system_alert "🚨 디스크 공간 부족" "디스크: $${DISK_USAGE}% (임계값: $${DISK_THRESHOLD}%)" 16711680 "backend"
}
